...
def run_hyperparameter_search(n_trials=100, output_dir="hyperparameter_tuning", clustering_mode="separate"):
    """
    ...
    # Prepare output directory: remove all files if exists, then (re)create
    exp_dir = os.path.join(output_dir, clustering_mode)
    ...
    min_cluster_sizes = [25, 50, 75, 100, 150]
    min_samples_ratios = [0.5, 0.7, 1.0]  # Ratio relative to min_cluster_size
    tfidf_weights = [0.3, 0.5, 0.7, 0.9]  # Weight for TF-IDF component in hybrid
    pca_components_options = [16, 32, 64, 128, 256]  # Number of PCA components or None to skip PCA
    ...
    for trial in range(n_trials):
        ...
        min_cluster_size = random.choice(min_cluster_sizes)
        ...
        dense_weight = round(1.0 - tfidf_weight, 1)  # Complement to make sum=1
        ...
        trial_dir = os.path.join(exp_dir, f"trial_{trial + 1:02d}")
        os.makedirs(trial_dir, exist_ok=True)
        
        # Save trial parameters immediately
        params_file = os.path.join(trial_dir, "trial_parameters.json")
    ...
    try:
        ...
        final_results_file = os.path.join(exp_dir, "hyperparameter_search_results.csv")
    ...
    try:
        if len(results_df) > 0:
            ...
            generate_analysis(results_df, exp_dir)
        else:
...
def run_single_trial(vectorization, sample_size, min_cluster_size, min_samples, 
                tfidf_weight=0.5, dense_weight=0.5, use_pca=True, pca_components=50, trial_dir=None, 
        ...
        try:
            ...
            if analyzer.vectorization == "tfidf":
                ...
                if 'vectorizer' in model_info:
                    all_vectorizers['combined'] = model_info['vectorizer']
                if 'pca' in model_info:
                    all_pca_models['combined'] = model_info['pca']
                ...
                if 'vectorizer' in hybrid_info:
                    all_vectorizers['combined'] = hybrid_info['vectorizer']
                if 'pca' in hybrid_info:
                    all_pca_models['combined'] = hybrid_info['pca']
            ...
            all_models['combined'] = clusterer
            
            # Save clustering results
            combined_dir = os.path.join(trial_dir, "combined")
            os.makedirs(combined_dir, exist_ok=True)
            analyzer.output_dir = combined_dir
        ...
        for dataset_name in analyzer.datasets:
            ...
            try:
                ...
                if analyzer.vectorization == "tfidf":
                    ...
                    if 'vectorizer' in model_info:
                        all_vectorizers[dataset_name] = model_info['vectorizer']
                    if 'pca' in model_info:
                        all_pca_models[dataset_name] = model_info['pca']
                elif analyzer.vectorization == "dense":
                    vectors, model_info = analyzer.vectorize_statements_dense(
                        statements,
                        use_pca=use_pca,
                    ...
                    # Store PCA model if used
                    if use_pca and 'pca' in model_info:
                        all_pca_models[dataset_name] = model_info['pca']
                else:  # hybrid
                    vectors, hybrid_info = analyzer.vectorize_statements_hybrid(
                        statements,
                    ...
                    if 'vectorizer' in hybrid_info:
                        all_vectorizers[dataset_name] = hybrid_info['vectorizer']
                    if use_pca and 'pca' in hybrid_info:
                        all_pca_models[dataset_name] = hybrid_info['pca']
                ...
                # Store the clustering model
                all_models[dataset_name] = clusterer
                
                # Create dataset-specific subdirectory within trial directory
                dataset_dir = os.path.join(trial_dir, dataset_name)
                os.makedirs(dataset_dir, exist_ok=True)
                
                # Temporarily change output directory to dataset-specific directory
                analyzer.output_dir = dataset_dir
                
                # Save clustering results to the dataset-specific directory
                analyzer.save_cluster_results(
                    statements, 
                    cluster_labels, 
                    "clusters",  # Use a simple prefix since we're already in dataset-specific dir
                    cluster_persistence=cluster_persistence
                )
                
                # Restore trial directory for other operations
                analyzer.output_dir = trial_dir
    ...
    # Save vectorizers
    if all_vectorizers:
        vectorizers_file = os.path.join(models_dir, "vectorizers.pkl")
        with open(vectorizers_file, 'wb') as f:
            pickle.dump(all_vectorizers, f)
        print(f"Saved vectorizers to {vectorizers_file}")
    
    # Save clustering models
    if all_models:
        models_file = os.path.join(models_dir, "clustering_models.pkl")
    ...
    # Save PCA models
    if all_pca_models:
        pca_file = os.path.join(models_dir, "pca_models.pkl")
        with open(pca_file, 'wb') as f:
            pickle.dump(all_pca_models, f)
        print(f"Saved PCA models to {pca_file}")
    ...
    try:
        ...
        save_trial_summary(trial_dir, trial_results, vectorization, sample_size, min_cluster_size, min_samples,
                         tfidf_weight=tfidf_weight, dense_weight=dense_weight, use_pca=use_pca, 
                         pca_components=pca_components, clustering_mode=clustering_mode)
...
def save_trial_summary(trial_dir, trial_results, vectorization, sample_size, min_cluster_size, min_samples,
                   tfidf_weight=0.5, dense_weight=0.5, use_pca=True, pca_components=50, clustering_mode="separate"):
    """
    ...
    with open(summary_file, 'w', encoding='utf-8') as f:
        ...
        if vectorization == "hybrid":
            ...
            f.write(f"  Dense Weight: {dense_weight}\n")
        f.write("\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, default="hyperparameter_results",
                        help="Base output directory for results (default: hyperparameter_results)")
    parser.add_argument("--clustering_mode", type=str, default="separate", choices=["separate", "combined"],
